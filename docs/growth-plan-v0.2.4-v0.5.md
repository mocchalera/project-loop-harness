...
| v0.5.0 Adoption | 契約安定性ポリシー、上流設計レイヤーの採用判断 | — | 初見 3 分で価値理解 |
...
- redundantなidle human gateを廃止し、明示intentは`pcl start`へ渡す;
...
4. コスト KPI 実測は v0.4.0 Dogfood Operations の位置を維持（テーゼ2 の
   証明を後退させない）。
5. Wave B 以降（3-command wedge、adaptive policy、Discovery Profile 等）は
   `docs/roadmap/integrated/agent-tasks-proposed/` の proposal に留め、
   wave 承認ごとに再採番して活性化する。v0.4 以降の version label は
   milestone review で exit 条件優先で決める。
