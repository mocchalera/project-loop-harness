...
再検証した。Zennでのfirst-channel publicationも別の人間承認後に検証済みである。
0175でstale editable install診断とCLI分割前の挙動契約を固定し、0176で
scale/event-log方針、0177でadvisory checkpoint routingまで進んだ。追加投稿、
real-provider実行、telemetryは引き続き個別の人間承認を要する。

## 12. v0.5.1 Trace & Efficient Handoff 再計画（2026-07-15）

Cockpit Ask `ask_8f99470fc206` への回答で、3人の外部利用者がいつ確保できるか
不明と確認された。外部利用者の確保を待つと開始時期が無期限になるため、
v0.5.1の開始条件から外す。`docs/launch/v0.5.0/feedback-study.md`は破棄せず、
参加者が得られた時点のAdoption evidenceとして別に扱う。

v0.5.1の正規Goalは、full transcriptを渡さず、hash/path/line-rangeでboundされた
claims-not-facts indexから別session/runtimeが再開できることを証明することである。
dispatchは0178 contract/fixture freeze → 0179 source-binding validation → 0180
claim-bound context/resume → 0181 controlled evaluation → 0182 local RC → 別途の
人間publication判断 → 0183 public closeoutとする。

0181は最低10件、2つのowner-controlled repo、2つの独立sessionを固定条件とし、
別runtime/model実行は必要なauthorizationを得た範囲で最低4件含める。成功率80%以上、
意図的なbroken bindingのsafe stop 100%、critical trust-boundary violation 0を
RC gateとする。このdogfoodを外部採用・市場検証として表現してはならない。

詳細なExit条件、測定定義、非目標は`docs/plan-v0.5.1.md`を正とする。schema 8、
CoreはLLMを呼ばない、full transcript非inline、no-index互換を維持し、first-class
Trace/Intent entity、telemetry、semantic retrievalは導入しない。
