...
| PF-12 | 52 markers total (> count limit 50); 23 stage dirs (> limit 20); **23 isolated clones (> limit 20), exactly one of them > 7 d old** (`$TMPDIR/pcl-finish-workspace-old1`, seeded with deterministic mtimes/paths) | `markers.over_count_overflow` = 2 oldest-beyond-limit; `stage_dirs.over_count_overflow` = 3 oldest-beyond-limit; `clones.over_age` = [`pcl-finish-workspace-old1`] and `clones.over_count_overflow` = that same path plus the next two oldest, ordered age-desc then path-asc (within a class `over_age` ⊆ `over_count_overflow` whenever both fire, because over-age items are the oldest); each class reports its own `candidate_count` as the size of its deduplicated union |
