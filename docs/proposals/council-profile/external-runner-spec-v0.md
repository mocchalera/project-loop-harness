...
```bash
pcl profile fixture-run \
  --request request.json \
  --status needs_human \
  --output-dir /tmp/output
```

このfixture helperはprovider code、network、API key、paid serviceを使わず、
PLH stateも変更しない。同じrequest bytesとstatusから同じlisted artifact
bytesとbundle digestを生成する。

Requestのproject fingerprintにはGit HEADが含まれる。外部runの間にcommitが
進むとingestは`request_project_fingerprint_mismatch`で停止するため、新しいHEADで
`pcl profile prepare`からやり直す。これは意図したfreshness gateである。
