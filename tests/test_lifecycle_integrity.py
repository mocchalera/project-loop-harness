...
def _feature_story_test(root: Path, capsys, *, approve: bool = True) -> tuple[str, str, str]:
    assert main(["--root", str(root), "feature", "add", "--name", "Integrity", "--surface", "cli:pcl"]) == 0
    feature_id = capsys.readouterr().out.strip()
    assert main([
        "--root", str(root), "story", "draft", "--feature", feature_id,
        "--actor", "agent", "--goal", "prove completion",
        "--expected-behavior", "terminal state is evidence-backed",
    ]) == 0
    ...
    assert main([
        ...
        "--type", "acceptance", "--scenario", "integrity gate", "--expected", "guarded",
    ]) == 0
...
def _adhoc_evidence(root: Path, capsys, text: str = "passed\n") -> str:
    ...
    assert main([
        "--root", str(root), "evidence", "add", "--file", "result.txt",
        "--summary", "acceptance output", "--copy", "--json",
    ]) == 0
...
def test_terminal_evidence_id_direct_route_and_links(tmp_path: Path, capsys) -> None:
    ...
    assert main([
        "--root", str(tmp_path), "test", "pass", test_id, "--summary", "passed",
    ...
    assert main([
        "--root", str(tmp_path), "feature", "status", feature_id, "--status", "done",
        "--summary", "complete", "--evidence-id", evidence_id, "--json",
    ]) == 0
    done = _json(capsys)
    ...
    try:
        links = {
            ...
            for row in conn.execute("SELECT target_type, target_id, link_role FROM evidence_links WHERE evidence_id = ?", (evidence_id,))
        }
    finally:
        conn.close()
...
def test_test_pass_without_story_rejects_with_zero_mutation(tmp_path: Path, capsys) -> None:
    _init(tmp_path, capsys)
    assert main(["--root", str(tmp_path), "feature", "add", "--name", "Unlinked", "--surface", "cli:pcl"]) == 0
    capsys.readouterr()
    assert main([
        "--root", str(tmp_path), "test", "plan", "--feature", "F-0001",
        "--type", "acceptance", "--scenario", "unlinked", "--expected", "reject",
    ]) == 0
    capsys.readouterr()
    evidence_id = _adhoc_evidence(tmp_path, capsys)
    before = _counts(tmp_path)
    assert main([
        "--root", str(tmp_path), "test", "pass", "TC-0001", "--summary", "passed",
        "--evidence-id", evidence_id, "--json",
    ]) == 2
...
def test_drifted_evidence_is_rejected_before_mutation(tmp_path: Path, capsys) -> None:
    ...
    evidence_id = _adhoc_evidence(tmp_path, capsys)
    (tmp_path / ".project-loop" / "evidence" / "adhoc-files" / evidence_id.lower() / "01-result.txt").write_text("changed\n", encoding="utf-8")
    before = _counts(tmp_path)
...
def test_no_evidence_test_transition_has_no_legacy_warning_or_mode(tmp_path: Path, capsys) -> None:
    ...
    _, _, test_id = _feature_story_test(tmp_path, capsys)
    assert main([
        "--root", str(tmp_path), "test", "block", test_id, "--summary", "blocked", "--json",
    ]) == 0
    payload = _json(capsys)
    assert "warnings" not in payload
    assert "evidence_mode" not in payload
    events = [json.loads(line) for line in (tmp_path / ".project-loop" / "events.jsonl").read_text(encoding="utf-8").splitlines()]
...
def test_feature_done_guard_rejects_incomplete_lifecycle_without_mutation(tmp_path: Path, capsys) -> None:
    _init(tmp_path, capsys)
...
def test_lifecycle_validation_is_advisory_without_policy_and_enforced_with_policy(tmp_path: Path, capsys) -> None:
    _init(tmp_path, capsys)
...
def test_legacy_raw_goal_closure_is_reported_and_new_raw_close_rejects(tmp_path: Path, capsys) -> None:
    _init(tmp_path, capsys)
    config = tmp_path / "pcl.yaml"
    config.write_text(config.read_text(encoding="utf-8").replace("\nvalidation:\n  lifecycle_integrity: enforced\n", "\n"), encoding="utf-8")
    ...
    before = _counts(tmp_path)
    assert main([
        "--root", str(tmp_path), "goal", "close", "G-0001", "--summary", "raw",
        "--evidence", "reports/result.md", "--json",
    ]) == 2
    ...
    try:
        conn.execute(
            ...
            (json.dumps({"closure": {"summary": "legacy", "evidence": "reports/result.md", "verification_id": None}}),),
        )
        conn.commit()
    finally:
        conn.close()
...
def test_goal_close_accepts_same_target_completed_packet_and_rejects_cross_target(tmp_path: Path, capsys) -> None:
    _init(tmp_path, capsys)
    ...
    before = _counts(tmp_path)
    assert main([
        "--root", str(tmp_path), "goal", "close", "G-0002", "--summary", "done",
        "--evidence-id", evidence_id, "--json",
    ]) == 2
    assert _json(capsys)["error"]["code"] == "goal_close_verification_required"
    assert _counts(tmp_path) == before
    ...
    assert main([
        "--root", str(tmp_path), "goal", "close", "G-0001", "--summary", "done",
        "--evidence-id", evidence_id, "--json",
    ]) == 0
    closed = _json(capsys)
    ...
    capsys.readouterr()
    dashboard = json.loads((tmp_path / ".project-loop" / "dashboard" / "dashboard-data.json").read_text(encoding="utf-8"))
...
def _completion_packet(root: Path, *, goal_id: str, outcome: str) -> dict:
    ...
    packet = {
        ...
        "outcome": outcome,
        "target": {"type": "goal", "id": goal_id, "intent": "Close direct goal", "work_brief_ref": None},
        "repository": {"base_revision": "abc1234", "head_revision": "def5678", "diff_sha256": "sha256:" + "0" * 64, "dirty": False},
        "changes": [{"path": "src/example.py", "change_type": "modified", "previous_path": None}],
        "checks": [], "claims": [], "unverified_claims": [], "risks": [], "human_decisions": [],
        "next_action": None,
    }
