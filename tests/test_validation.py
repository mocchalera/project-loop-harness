    ...
    assert result.to_dict()["findings"] == [
        {
            ...
            "entity": {"type": "project", "id": str(tmp_path)},
            "related": [],
...
def _valid_rubric(evidence_id: str | None = None) -> dict:
    return {
        ...
        "acceptance_criteria": [
            {
                "criterion": "Expected behavior was verified",
                "met": "yes",
                "evidence_id": evidence_id,
            }
        ],
...
def _create_closed_defect(root: Path, capsys) -> None:
    ...
    assert (
        main(["--root", str(root), "feature", "add", "--name", "Login", "--surface", "ui:/login"])
        == 0
    )
    assert (
        main(
    ...
    assert (
        main(["--root", str(root), "loop", "complete", "WR-0001", "--summary", "Repair passed"])
        == 0
    )
    assert (
        main(
...
def _create_terminal_test_case(root: Path, capsys, transition: str) -> None:
    ...
    if transition == "pass":
        ...
        assert (
            main(["--root", str(root), "loop", "run", "feature_coverage", "--goal", "G-0001"]) == 0
        )
        assert (
            main(
                [
...
def test_strict_validate_rejects_closed_goal_without_proof(tmp_path: Path, capsys) -> None:
    ...
    _update_db(
        tmp_path, "UPDATE goals SET status = 'closed', completion_json = '{}' WHERE id = 'G-0001'"
    )
...
def test_validate_warns_and_strict_errors_on_invalid_rubric_v1(tmp_path: Path, capsys) -> None:
    ...
    assert (
        "Verification V-0001 rubric/v1 invalid: acceptance_criteria is required."
        in normal["warnings"]
    )
...
def test_strict_validate_rejects_verified_defect_without_evidence_or_verification(
    ...
    _update_db(
        tmp_path, "UPDATE defects SET status = 'verified', evidence_id = NULL WHERE id = 'D-0001'"
    )
    ...
    assert (
        "Defect D-0001 is verified but has no approved verification tied to the defect."
        in payload["errors"]
    )
...
def test_strict_validate_rejects_closed_defect_missing_fix_evidence(tmp_path: Path, capsys) -> None:
    ...
    _update_db(
        tmp_path, "DELETE FROM events WHERE entity_id = 'D-0001' AND event_type = 'defect_fixed'"
    )
...
def test_strict_validate_rejects_closed_defect_missing_close_evidence(
    ...
    _update_db(
        tmp_path, "DELETE FROM events WHERE entity_id = 'D-0001' AND event_type = 'defect_closed'"
    )
    ...
    assert (
        main(
            [
                ...
                "--expected-behavior",
                "passing tests link reviewed behavior",
            ]
        )
        == 0
    )
    assert (
        main(["--root", str(tmp_path), "story", "approve", "US-0001", "--summary", "reviewed"]) == 0
    )
...
def test_strict_validate_rejects_terminal_test_case_wrong_evidence_type(
    tmp_path: Path, capsys
) -> None:
...
def test_strict_validate_rejects_terminal_test_case_missing_transition_event(
    ...
    _update_db(
        tmp_path,
        "DELETE FROM events WHERE entity_id = 'TC-0001' AND event_type = 'test_case_waived'",
    )
...
def test_strict_validate_rejects_duplicate_active_runs_for_same_target(
    tmp_path: Path, capsys
) -> None:
    assert main(["init", "--target", str(tmp_path)]) == 0
    assert main(["--root", str(tmp_path), "goal", "create", "--title", "Coverage"]) == 0
    ...
    assert (
        main(["--root", str(tmp_path), "loop", "run", "regression_loop", "--goal", "G-0001"]) == 0
    )
    capsys.readouterr()
...
def test_strict_validate_rejects_duplicate_active_runs_for_same_defect(
    tmp_path: Path, capsys
) -> None:
    assert main(["init", "--target", str(tmp_path)]) == 0
    assert (
    ...
    assert (
        main(["--root", str(tmp_path), "loop", "run", "defect_repair", "--defect", "D-0001"]) == 0
    )
    assert (
        main(["--root", str(tmp_path), "loop", "run", "defect_repair", "--defect", "D-0001"]) == 0
    )
    ...
    payload = _json_output(capsys)
    assert (
        "Duplicate active workflow runs for defect D-0001: WR-0001, WR-0002." in payload["errors"]
    )
...
def test_strict_validate_rejects_terminal_goal_with_non_terminal_task(
    ...
    payload = _json_output(capsys)
    assert (
        "Terminal goal G-0001 is closed but has non-terminal task T-0001 (todo)."
        in payload["errors"]
    )
...
def test_strict_validate_rejects_terminal_workflow_run_with_active_job(
    ...
    payload = _json_output(capsys)
    assert (
        "Terminal workflow run WR-0001 is failed but has active agent job J-0001 (queued)."
        in payload["errors"]
    )
...
def test_strict_validate_rejects_decision_block_link_to_missing_entity(
    ...
    payload = _json_output(capsys)
    assert (
        "Decision DEC-0001 blocks_json references missing workflow_run WR-9999."
        in payload["errors"]
    )
...
def test_strict_validate_rejects_code_context_link_target_binding_mismatch(
    ...
    assert any(
        ...
        "target_binding that disagrees with the evidence link routing row" in error
        for error in payload["errors"]
    )
    finding = next(
        item for item in payload["findings"] if item["code"] == "provenance_target_binding_mismatch"
    )
    assert finding["entity"] == {"type": "evidence", "id": "E-0001"}
    assert finding["related"] == [{"type": "task", "id": "T-0001"}]
    assert finding["suggested_commands"] == [
        "pcl --json evidence show E-0001",
        "pcl --json audit check",
    ]
...
def test_strict_validate_rejects_evidence_link_to_missing_evidence(
    ...
    payload = _json_output(capsys)
    assert any(
        "Evidence link E-9999 to future_target:FT-9999" in error for error in payload["errors"]
    )
...
def test_strict_validate_rejects_missing_local_evidence_artifact(
    ...
    before = {
        ...
        for path in sorted((tmp_path / ".project-loop").rglob("*"))
        if path.is_file()
    }
    ...
    payload = _json_output(capsys)
    assert (
        "Evidence E-0001 path does not exist: .project-loop/evidence/agent-runs/J-0001/output.md."
        in payload["errors"]
    )
    assert (
        "Agent job J-0001 output_path does not exist: .project-loop/evidence/agent-runs/J-0001/output.md."
        in payload["errors"]
    )
    missing = [item for item in payload["findings"] if item["code"] == "artifact_missing"]
    assert [item["entity"] for item in missing] == [
        {"type": "evidence", "id": "E-0001"},
        {"type": "agent_job", "id": "J-0001"},
    ]
    assert [item["suggested_commands"] for item in missing] == [
        ["pcl --json evidence show E-0001"],
        ["pcl --json jobs read J-0001"],
    ]
    assert [
        item["message"] for item in payload["findings"] if item["severity"] == "error"
    ] == payload["errors"]
    assert main(["--root", str(tmp_path), "validate", "--strict", "--json"]) == 1
    assert _json_output(capsys) == payload
    after = {
        path.relative_to(tmp_path).as_posix(): path.read_bytes()
        for path in sorted((tmp_path / ".project-loop").rglob("*"))
        if path.is_file()
...
def test_strict_validate_accepts_valid_closed_goal_and_defect(tmp_path: Path, capsys) -> None:
    ...
    assert (
        main(
            ["--root", str(tmp_path), "loop", "complete", "WR-0001", "--summary", "Coverage passed"]
        )
        == 0
    )
    assert (
    ...
    assert (
        main(["--root", str(tmp_path), "defect", "triage", "D-0001", "--summary", "Triaged"]) == 0
    )
    ...
    assert (
        main(["--root", str(tmp_path), "loop", "complete", "WR-0002", "--summary", "Repair passed"])
        == 0
    )
    assert (
        main(
