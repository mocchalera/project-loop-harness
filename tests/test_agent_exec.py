...
from pcl.agent_exec import FAIL_MAX_BYTES, FAIL_MAX_LINES, gc_agent_exec
...
def test_secret_shaped_argv_and_output_are_redacted(
    ...
    assert metadata["command"][-1] == REDACTED_SECRET
...
def test_long_argv_is_bounded_in_machine_result(
    ...
    exit_code = cli_main(
        ["--json", *_run_python("raise SystemExit(0)", long_argument)]
    )
