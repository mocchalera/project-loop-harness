...
from .redaction import REDACTED_SECRET, compile_redaction_patterns, redact_text
...
def run_agent_exec(
    ...
    *,
    ...
    patterns = compile_redaction_patterns(redaction_patterns)
    root = _prepare_state_root(state_root or default_agent_exec_state_root())
    run_id = _new_run_id()
    run_dir = _create_run_directory(root, run_id)
    redacted_command, command_redacted = _redact_argv(argv, patterns)
    ...
    with tempfile.TemporaryDirectory(prefix="pcl-agent-exec-") as temp_name:
        ...
        diagnostic = (
            ...
            if status == "PASS"
            else _select_diagnostic(execution)
        )
    ...
    result: dict[str, Any] = {
        ...
        "redacted": bool(execution.get("redacted") or command_redacted),
...
def _redact_argv(
    argv: list[str], patterns: tuple[Pattern[str], ...]
) -> tuple[list[str], bool]:
    redacted: list[str] = []
    changed = False
    redact_next = False
    for item in argv:
        if redact_next:
            ...
            redact_next = False
            continue
        value, item_changed = redact_text(item, additional_patterns=patterns)
...
def _select_diagnostic(execution: dict[str, Any]) -> DiagnosticSelection:
        ...
        for index, line in enumerate(lines):
            ...
            for context_index in range(max(0, index - 2), min(len(lines), index + 3)):
                ...
                if key in seen:
                    continue
                seen.add(key)
                selected.append(f"{stream_name} | {lines[context_index]}")
    if selected:
    ...
    if stderr_lines:
        labeled = [f"STDERR | {line}" for line in stderr_lines[-(FAIL_MAX_LINES - 4):]]
        text, truncated = _bound_lines(
    ...
    combined: list[str] = []
    for stream_name, lines in streams:
        combined.extend(f"{stream_name} | {line}" for line in lines[-50:])
    if combined:
        text, truncated = _bound_lines(
