...
v0.5.0 Council Profile feature track: see `docs/plan-v0.5.0-council-profile.md`. Dispatch is 0154 ADR/proposal contract freeze → 0155 runtime validators/built-in registry → 0156 read-only prepare → 0157 dry-run validator → 0158 atomic Evidence ingest/audit extension → 0159 existing Decision/human provenance/run authorization and legacy-bypass guard → 0160 offline E2E → 0161 offline dogfood/Skill parity → 0162 frozen offline evaluation/adoption gate. The human outcome is `continue experiment`; second-repository and real-provider work remain parked behind explicit authorization. The built-in-only MVP keeps schema 8 and never invokes provider code from Core. 0163 activates the separately numbered Adoption/Distribution track before v0.5.0 publication.
